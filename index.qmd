...
author: Fara  # Remember to add your name!
theme: united
---
...
cantril_table<-cantril %>% select(Country, Year, Cantril_ladder_score="Cantril ladder score") %>% 
filter(Year==2011 | Year== 2023)  
cantril_table_wider<-cantril_table %>% pivot_wider(names_from = Year, values_from = Cantril_ladder_score)
  
 cantril_table_wider<- cantril_table_wider %>% mutate(
   Absolute_change=round((`2023`-`2011`),2),
   Relative_change=round((`2023`-`2011`)/`2011`, 3)) %>% 
   mutate(Relative_change=percent(Relative_change, accuracy = 0.1)) %>% 
   rename("Absolute change"=Absolute_change, "Relative change"=Relative_change)
 cantril_table_wider %>% 
 reactable()
...
cantril_chart<-cantril %>% filter(Country== c("Afghanistan", "Congo", "China", "Costa Rica"))

countries_chart<-c("Afghanistan", "Congo", "China", "Costa Rica")
cantril_chart<-cantril %>% filter(Country %in% countries_chart) %>% 
